...
content = re.sub(
    r"\.select\('user_id, first_name, last_name'\)",
    r".select('user_id, first_name, last_name, user_roles(roles(role_name))')",
    content
)

# 2. Add isStaff flag and modify log insertion
target_logic = r"""      const startOfDay = new Date\(\);
      startOfDay\.setHours\(0,0,0,0\);.*?await supabase\.from\('system_logs'\)\.insert\(\{.*?user_name: `\$\{user\.first_name\} \$\{user\.last_name\}`\s*\}\);"""
...
content = re.sub(target_logic, replacement_logic, content, flags=re.DOTALL)
