...
def replace_delete(filepath):
    ...
    target = r"  async function handleDelete\(id: string\) \{.*?fetchUsers\(\);\n  \}"
    ...
    const { data: classData } = await supabase.from('classes').select('class_id').or(`primary_teacher_id.eq.${id},co_teacher_id.eq.${id}`);
