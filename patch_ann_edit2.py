...
target = """  const handleEditAnnouncementSub = async (annId: string, authorRole: string, isSystem: boolean) => {
...
target2 = """<button onClick={() => { handleEditAnnouncementSub(ann.announcement_id, authorRole, isSystem); setSelectedPostingRole(""); }} className="bg-primary text-on-primary px-5 py-2 rounded-full font-label font-bold text-sm">Save</button>"""
