...
// Update students fetch
const oldFetch = `    const { data, error } = await supabase.from('enrollments')
      .select(\`
         student_id,
...
const newFetch = `    const { data: enrollData, error } = await supabase.from('enrollments')
      .select('student_id')
      .eq('class_id', cls.class_id);
